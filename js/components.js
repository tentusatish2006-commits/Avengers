...
function buildSidebar(active) {
  ...
  SR_SIDEBAR_SECTIONS.forEach(function (sec) {
    ...
    sec.items.forEach(function (it) {
      ...
      html += '<span class="nav-label" data-i18n="' + it.key + '">' + it.label + '</span></a>';
    });
  });
  html += '</nav>';
  ...
  html += '<span class="nav-label">Sign Out</span></button></div></aside>';
  return html;
}
  ...
  return '' +
    '<div class="topbar-right" style="display:flex;align-items:center;gap:10px;margin-left:auto;flex-shrink:0;">' +
      ...
      '<a href="alerts.html" id="sr-alerts-btn" title="Alerts" style="position:relative;width:42px;height:42px;border-radius:11px;display:inline-flex;align-items:center;justify-content:center;background:rgba(255,149,0,0.15);border:1px solid rgba(255,149,0,0.5);text-decoration:none;font-size:1.2rem;">' +
        '🔔<span style="position:absolute;top:5px;right:5px;width:9px;height:9px;border-radius:50%;background:#ff3b3b;border:1.5px solid #0a1628;"></span></a>' +
      '<a href="settings.html" id="sr-settings-btn" title="Settings" style="width:42px;height:42px;border-radius:11px;display:inline-flex;align-items:center;justify-content:center;background:rgba(0,212,255,0.12);border:1px solid rgba(0,212,255,0.45);text-decoration:none;font-size:1.2rem;">⚙</a></div>';
}
...
function ensureTopbar(opts) {
  ...
  if (existing) {
    ...
    if (!document.getElementById('sr-menu')) {
      ...
      if (left) left.insertAdjacentHTML('afterbegin', '<button type="button" class="icon-btn" id="sr-menu" aria-label="Menu" style="width:38px;height:38px;border-radius:9px;border:1px solid rgba(0,212,255,0.3);background:rgba(0,212,255,0.1);color:#e8f4ff;cursor:pointer;font-size:1.2rem;">☰</button>');
    }
    return;
  }
...
function initSharedComponents(opts) {
  ...
  try {
    ...
    if (page === 'reports.html' || page === 'reports') {
      ...
      s.src = 'js/report-assign-multi.js?v=1';
      s.async = true;
