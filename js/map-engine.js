...
(function (global) {
  ...
  var ROUTE_DEFS = [
    { id: 'SAFE-1', name: 'Guwahati to Shillong (Safe)', status: 'safe', color: '#00ff88', weight: 6,
      waypoints: ['Guwahati', 'Nongpoh', 'Shillong'] },
    { id: 'MOD-1', name: 'Guwahati to Silchar to Aizawl (Alternate)', status: 'moderate', color: '#ff9500', weight: 5,
      waypoints: ['Guwahati', 'Nagaon', 'Silchar', 'Aizawl'] },
    { id: 'CRIT-1', name: 'Guwahati to Tawang (Critical)', status: 'critical', color: '#ff3b3b', weight: 5,
      waypoints: ['Guwahati', 'Tezpur', 'Bomdila', 'Tawang'] },
    { id: 'SAFE-2', name: 'Agartala to Silchar (Safe)', status: 'safe', color: '#00ff88', weight: 5,
      waypoints: ['Agartala', 'Silchar'] },
    { id: 'MOD-2', name: 'Dimapur to Kohima (Alternate)', status: 'moderate', color: '#ff9500', weight: 5,
      waypoints: ['Dimapur', 'Kohima'] },
    { id: 'CRIT-2', name: 'Jorhat to Itanagar (Critical)', status: 'critical', color: '#ff3b3b', weight: 5,
  ...
  function initMap(containerId, opts) {
    ...
    L.tileLayer(MAP_CONFIG.tileUrl, { maxZoom: 18, attribution: 'Leaflet | SmartRoute NER' }).addTo(map);
    MapEngine.map = map;
  ...
  function densifyRoadLike(waypoints) {
    ...
    for (var i = 0; i < waypoints.length - 1; i++) {
      ...
      for (var s = 0; s <= steps; s++) {
        ...
        var lng = a[1] + lngSpan * t + offset * (latSpan === 0 ? 0.2 : 0);
        out.push([lat, lng]);
      }
    }
    return out;
  ...
  function vehicleIcon(color) {
    return L.divIcon({
      className: '',
      html: '<div style="width:22px;height:22px;border-radius:6px;background:' + color + ';border:2px solid #fff;box-shadow:0 2px 6px rgba(0,0,0,.4);display:flex;align-items:center;justify-content:center;font-size:11px;">&#128666;</div>',
